...
class SunshineCart {
	...
	function add_to_cart( $image_id, $product_id, $qty, $price_level, $comments='', $type='', $extra = '' ) {
		...
		$item = array(
			...
			'taxable' => get_post_meta( $product_id, 'sunshine_product_taxable', true ),
			'hash' => md5( time() )
		);
		if ( is_array( $extra ) ) {
		...
		$can_add_item = apply_filters( 'sunshine_can_add_to_cart_item', true, $item );
		if ( !$can_add_item ) {
			return false;
		}
		...
		$taxable = get_post_meta( $product_id, 'sunshine_product_taxable', true );
		if ( $taxable && $sunshine->options['display_price'] == 'with_tax' ) {
			if ( $sunshine->options['price_has_tax'] == 'no' ) {
				$item['price_with_tax'] = $item['price'] + ( $item['price'] * ( $sunshine->options['tax_rate'] / 100 ) );
			} else {
				$item['price_with_tax'] = $item['price'];
			}
	...
	*/
	...
	public function set_total() {
		...
		if ( $sunshine->options['price_has_tax'] != 'yes' ) {
			$this->total += $this->tax;
		}
		$this->total += $this->shipping_method['cost'];
	...
	public function get_product_display_price( $product_id, $price_level, $formatted = true ) {
		...
		if ( $sunshine->options['display_price'] == 'with_tax' && $sunshine->options['price_has_tax'] == 'no' ) {
			...
			$taxable = get_post_meta( $product_id, 'sunshine_product_taxable', true );
			$hide_suffix = true;
			if ( $taxable && $sunshine->options['display_price'] == 'with_tax' && $sunshine->options['price_has_tax'] == 'no' ) {
				$price = $price + ( $price * ( $sunshine->options['tax_rate'] / 100 ) );
				$result = sunshine_money_format( $price, false );
				$hide_suffix = false;
			}
			if ( $formatted ) {
				$result = sunshine_money_format( $price, false, $hide_suffix );
			} else {
				$result = $price;
			}
		} else {
	...
	function set_number_format() {
		$this->subtotal = sunshine_fix_number( $this->subtotal );
		$this->tax = sunshine_fix_number( $this->tax );
		$this->shipping_method['cost'] = sunshine_fix_number( $this->shipping_method['cost'] );
		$this->discount_total = sunshine_fix_number( $this->discount_total );
		$this->total = sunshine_fix_number( $this->total );
	}
