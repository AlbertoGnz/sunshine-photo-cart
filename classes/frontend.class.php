...
class SunshineFrontend extends SunshineSingleton {
	...
	function __construct() {
		...
		if ( $sunshine->options['theme'] == 'theme' ) {
			if ( !isset( $sunshine->options['use_shortcode'] ) || !$sunshine->options['use_shortcode'] ) {
	...
	static function sunshine_content( $content = '' ) {
	...
	*/
	...
	static function pluralize( $count, $singular, $plural, $echo=true ) {
		if ( $count == 1 )
			$output = $singular;
		else
			$output = $plural;
...
?>
	...
	function can_use_cart() {
		global $sunshine;
		if ( ( isset( $sunshine->options['proofing'] ) && $sunshine->options['proofing'] ) && ( is_page( $sunshine->options['page_cart'] ) || is_page( $sunshine->options['page_checkout'] ) ) ) {
