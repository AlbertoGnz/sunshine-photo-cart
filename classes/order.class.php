...
/* 		if ( $data['tax_shipping'] ) {
			...
			$items_html .= ' <tr><td colspan="4" style="'.$td_style.' text-align: right;">' . __('Shipping', 'sunshine') . ' ('.sunshine_get_shipping_method_name( $data['shipping_method'] ).')</td><td style="'.$td_style.'">'.sunshine_money_format( $data['shipping_cost'], false, true ).'</td></tr>';
		} */
/* 		if ( $sunshine->options['display_price'] != 'with_tax'  ) {
		...
		if ( $data['discount_total'] > 0 ) {
			$items_html .= ' <tr><td colspan="4" style="'.$td_style.' text-align: right;">' . __('Discounts', 'sunshine') . '</td><td style="'.$td_style.'">'.sunshine_money_format( $data['discount_total'], false, true ).'</td></tr>';
		}
		if ( $data['credits'] ) {
		...
		if ( isset( $data['email'] ) || isset( $data['address'] ) || isset( $data['address2'] ) || isset( $data['city'] ) || isset( $data['state'] ) || isset( $data['zip'] ) ) {
