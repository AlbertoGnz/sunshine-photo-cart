		...
		<tr class="item">
			...
			<td class="image">
				<?php
					...
					if ( $item['image_id'] > 0 && $item['type'] == 'image' ) {
			...
			<td class="price">
				...
				if ( empty( $item['price_with_tax'] ) ) {
					...
					sunshine_money_format( $item['price_with_tax'], true, true );
				}
				?>
			</td>
			<td class="total">
...
function sunshine_orders_save_postdata( $post_id ) {
	...
	$new_order_status = sanitize_text_field( $_POST['sunshine_order_status'] );
	$terms = wp_get_post_terms( $post_id, 'sunshine-order-status' );
	if ( $terms[0]->slug != $new_order_status ) {
		wp_set_post_terms( $post_id, $new_order_status, 'sunshine-order-status' );
		...
		do_action( 'sunshine_new_order_status_' . $new_order_status, $post_id );
		do_action( 'sunshine_change_order_status_' . $terms[0]->slug . '_to_' . $new_order_status, $post_id );
		do_action( 'sunshine_change_order_status_' . $terms[0]->slug . '_to_anything', $post_id );
		do_action( 'sunshine_change_order_status_not_' . $terms[0]->slug, $post_id );
	}
...
function sunshine_order_columns_content( $column, $post_id ) {
	...
	case 'customer':
		...
		$data = maybe_unserialize( get_post_meta( $post_id, '_sunshine_order_data', true ) );
		echo $data['email'] . ' ' . $data['last_name'];
		if ( $customer_id ) {
	...
	case 'total':
		...
		sunshine_money_format( $order_data['total'], true, true );
		if ( $order_data['credits'] > 0 ) {
			echo ' <small style="color: #999;">(' . sunshine_money_format( $order_data['credits'], false, true ) . ' ' . __( 'in credits', 'sunshine' ) . ')</small>';
		}
		break;
	case 'date_ordered':
