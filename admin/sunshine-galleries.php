...
function sunshine_gallery_meta_boxes() {
	global $post;
	$image_count = 0;
	$images = get_children( array( 'post_parent' => $post->ID, 'post_type' => 'attachment', 'post_mime_type' => 'image/jpeg' ) );
...
add_action( 'admin_head', 'sunshine_parent_gallery_settings_notice' );
function sunshine_parent_gallery_settings_notice() {
	global $post_type;
	...
	jQuery( document ).ready( function($) {
		$( '#parent_id' ).change( function(){
			...
			if ( gallery_parent_id > 0 ) {
				$( '#parent_id' ).after( '<div id="parent-gallery-setting-notice"><?php esc_js( _e( 'Please note that this gallery will not inherit the settings from the selected parent gallery', 'sunshine' ) ); ?>');
			} else {
