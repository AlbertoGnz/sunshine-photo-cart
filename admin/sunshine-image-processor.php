			...
			function sunshine_file_import(item_number) {
				...
				$.postq('sunshineimageprocessor', ajaxurl, data, function(response) {
                    ...
                    if ( jqXHR.status == 500 || jqXHR.status == 0 ){
                        $( '#errors' ).append( '<li><strong><?php _e( 'Cannot process image, likely out of memory', 'sunshine' ); ?></strong></li>' );
                    }
                });
			}
			for (i = 1; i <= total; i++) {
...
function sunshine_ajax_file_save() {
	...
	if ( !is_wp_error( $attachment_id ) ) {
        ...
        echo json_encode( array( 'status' => 'error', 'file' => $file_name, 'error' => __( 'Could not upload', 'sunshine' ) ) );
    }
